...
public class EditTaskCommandParserTest {
    ...
    public void parse_missingFields_failure() {
        ...
        assertParseFailure(parser, INDEX_FIRST_PROJECT.getOneBased() + TASK_DELETE_1, MESSAGE_INVALID_FORMAT);
        assertParseFailure(parser, INDEX_FIRST_PROJECT.getOneBased() + TASK_DESC_IMPLEMENT_X,
                MESSAGE_INVALID_FORMAT);
    }

    @Test
    public void parse_invalidValues_failure() {
        assertParseFailure(parser, "1*" + TASK_DELETE_1 + TASK_DESC_IMPLEMENT_X, MESSAGE_INVALID_FORMAT);
        assertParseFailure(parser, INDEX_FIRST_PROJECT.getOneBased() + " -i x" + TASK_DESC_IMPLEMENT_X,
                ParserUtil.MESSAGE_INVALID_INDEX);
        assertParseFailure(parser, INDEX_FIRST_PROJECT.getOneBased() + TASK_DELETE_1 + INVALID_TASK_DESC_ADD,
                Task.MESSAGE_CONSTRAINTS);
    }

    @Test
    public void parse_duplicatePrefixes_failure() {
        assertParseFailure(parser, INDEX_FIRST_PROJECT.getOneBased() + " -i 1 -i 2" + TASK_DESC_IMPLEMENT_X,
                Messages.MESSAGE_DUPLICATE_FIELDS + PREFIX_INDEX);
        assertParseFailure(parser, INDEX_FIRST_PROJECT.getOneBased() + TASK_DELETE_1 + TASK_DESC_IMPLEMENT_X
                        + " -n another task", Messages.MESSAGE_DUPLICATE_FIELDS + PREFIX_NAME);
    }

    @Test
    public void parse_validArgs_success() {
        String userInput = INDEX_FIRST_PROJECT.getOneBased() + TASK_DELETE_1 + TASK_DESC_IMPLEMENT_X;
        EditTaskCommand expectedCommand = new EditTaskCommand(INDEX_FIRST_PROJECT,
